<template>
                    ...
                    <template v-for="(item, i) in tableActions" :key="i">
                        <a v-if="(!isEmpty(item.config) && record[item.config?.key] === item?.config?.value) || isEmpty(item.config)"
                            ...
                            <template v-if="!item.labelShow">
                                <EditOutlined v-if="item.key === 1" />
                                <FileSearchOutlined v-else-if="item.key === 2" />
                                <DeleteOutlined v-else-if="item.key === 3" />
                            </template>
                            <template v-if="item.labelShow">
                                {{ item.label }}
                            </template>
    ...
    <template v-if="!isEmpty(modal) || !modal?.hide">
        <a-modal class="gc-modal" :width="modal?.config?.width || 1200" v-bind="modalConfig" :open="open !== 0"
            destroy-on-close @ok="onModalOk" @cancel="onModalCancel">
...
import { Modal as AModal, Space as ASpace, message } from 'ant-design-vue'
import { EditOutlined, FileSearchOutlined, DeleteOutlined } from '@ant-design/icons-vue';
